Shader "Hidden/Custom/Glitch"
{
    ...
    // シェーダーのプロパティ
    float _IntensityBase;
    float _IntensityBeat;
    float _IntensityAudioSpectrum;
    ...
    float4 Frag(VaryingsDefault i): SV_Target
    {
        ...
        float intensity = _IntensityBase;
        intensity += _IntensityBeat * cos(_Beat * TAU);
        intensity += _IntensityAudioSpectrum * _AudioSpectrumLevels[0];

        // grid hash
        float2 hash = hash23(float3(floor(float2(uv.x * 32.0, uv.y * 32.0)), _Beat));

        // uv shift
        uv += _GlitchUvIntensity * intensity * (1.0 - 2.0 * hash);

        // distortion
        uv.x += _DistortionIntensity * intensity * sin(uv.y * 4.0 + _Beat);

        // rgb shift
        float angle = hash.x * TAU;
        float2 offset = _RgbShiftIntensity * intensity * float2(cos(angle), sin(angle));
        ...
        // noise
        color.rgb += _NoiseIntensity * intensity * hash12(float2(i.texcoord.y * 20.0, _Beat));
